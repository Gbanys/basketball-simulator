...
def train_layup_model():
    ...
    layup_dataframe = pd.DataFrame({'scaled_player_ability' : [scaled_player_ability[i][0] for i in range(0, len(scaled_player_ability))],
    ...
    layup_dataframe['shot_made_overall'] = layup_dataframe[['shot_made_by_ability', 'shot_made_by_energy', 'shot_made_by_on_ball_defending']]\
    .apply(lambda x: np.mean(x), axis=1)
    layup_dataframe['shot_made_overall'] = layup_dataframe['shot_made_overall'].apply(determine_if_shot_made)

    input_features = layup_dataframe[['scaled_player_ability', 'scaled_player_energy', 'scaled_on_ball_defending']]
    shot_made_overall = layup_dataframe['shot_made_overall']

    X_train, X_test, y_train, y_test = train_test_split(input_features, shot_made_overall)

    layup_model = LogisticRegression().fit(X_train, y_train)
    score = layup_model.score(X_test, y_test)

    print("Model training SUCCESSFUL:")
    print(f'Model score: {score}')

    pickle.dump(layup_model, open('models/layup_model.pkl', 'wb'))
